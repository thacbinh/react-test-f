...
export const updateUserAPI = (fullName: string, _id: string, phone: string) => {
    const urlBackend = "/api/v1/user";
...
export const getBookAPIPaginate = (current: number, pageSize: number) => {
    const urlBackend = `/api/v1/book?current=${current}&pageSize=${pageSize}`;
    return axios.get<IBackendRes<IModelPaginate<IBookTable>>>(urlBackend)
}
...
export const createOrderAPI = (name: string, address: string, phone: string, totalPrice: number, type: string, detail: IBookPayment[]) => {
